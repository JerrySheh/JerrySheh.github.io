...
**前面提到，大模型本质上只能输入输出自然语言，并不具备执行能力。有必要再强调一次，大模型唯一能做的事情，就是接收一段文本，然后预测并输出下一段文本。**

那么，AI 是怎么帮我们定闹钟、查天气的呢？

事实上，我们面对的各种 AI 助手，输入框背后不单纯只有大模型，而是一个 **AI Agent（智能体）**。AI Agent 本质上就是一个程序，一边对接了大模型，另一边对接了各种各样的外部资源（Skills、RAG、MCP Server、第三方接口等等）。

当我们提问或下达一个命令，AI Agent 和大模型配合流程如下：

1. 问题先进入 Agent，Agent 先通过 意图路由（Intent Routing）或 技能检索（Skill Retrieval）判断需不需要用到 RAG 和 Skill 
2. 将用户提问和需要用到的工具列表，组装成一个巨大的 Prompt，进入大模型
3. 大模型经过思考和拆解，如果需要调用外部接口，则返回 JSON ，否则直接返回自然语言回答
4. Agent 如果收到 JSON，则调用外部接口，再把结果重新组装到 Prompt 中再次给到大模型，如果收到自然语言回答，则直接返回给用户
