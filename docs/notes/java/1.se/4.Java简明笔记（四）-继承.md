...
tags:
  - Java
abbrlink: 2fJava80
createTime: 2018/01/23 18:14:26
permalink: /java/015lizce/
---
...
为什么同样是 new 对象， 两个 Object 返回 false， 两个 String 却返回 true 呢？ 原因是：String 重写了 equal() 方法，不是用 == 来判断的，而是比较值。具体看：[探究 String 类 equals 方法源码](/java/wzx0vqqp/#探究-string-类-equals-方法源码)
...
参考：[数据结构（六）查找——哈希函数和哈希表](/data-structure/7ahvzi83/)
...
当垃圾回收器确定不存在对该对象的更多引用时，由对象的垃圾回收器调用此方法。见 [Java虚拟机（四）垃圾回收策略](/java/ows4wboi/)。
