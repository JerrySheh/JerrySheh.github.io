...
permalink: /interview/point-to-offer/
---
...
如果不能修改输入的数组，我们可以用一个辅助数组，将原数组中数字 m 复制到辅助数组下标为 m 的地方，这样很容易发现重复数字。

## 变换二：不能修改输入的数组，且不能用辅助数组

利用二分法的思想，例如长度为8的数组，先看前4个数字在整个数组中出现的次数，如果大于4次，说明前4个数字里面一定有重复数字。再将前4个数字分成2组，第一组是前两个数字，第二组是后两个数字， 如果 1、2 在整个数组中出现大于2次，说明重复数字在 1、2 里面，再分别统计 1 和 2
...
思路：分为 stackA 和 stackB ，队列进时，往 stackA 进，队列出时先判断 StackB 是否为空，不为空直接出，为空先把 stackA 全部倒进 stackB，再出。
