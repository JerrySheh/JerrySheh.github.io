---
title: 查找算法（二分、哈希、查找树）
categories: 数据结构和算法
tags:
  - 数据结构和算法
abbrlink: c61c7436
createTime: 2018/05/01 17:59:57
permalink: /data-structure/7ahvzi83/
---
...
简单地说，哈希是一种把一大堆数据放到一小堆格子里的游戏。麦当劳早高峰时总喜欢在取餐处摆个架子，订单尾号为1的放在1号格子、订单尾号为2的放在2号格子......这就是一种哈希策略。可以预见，一个格子里可能存在多个订单，这就是哈希冲突。但也不用担心，我们仍可以先找到对应的架子，再根据我们的订单号“35018”拿到正确的早餐。
