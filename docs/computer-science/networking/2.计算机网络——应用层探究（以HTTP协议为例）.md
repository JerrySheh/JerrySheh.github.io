---
title: 计算机网络——应用层探究（以HTTP协议为例）
categories: 计算机网络
tags:
  - 计算机网络
createTime: 2018/03/08 11:54:26
permalink: /networking/application/
---
...
这个 html 页面是一个对象（通常为index.html），其中的 logo 图片也是一个对象。这些对象都存储在服务器上面，当你访问页面时再传输给你，对象一般都可以通过 URL 寻址，比如谷歌首页的 Logo 图片，其 URL 地址是：
`https://www.google.com/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png`

HTTP 定义了客户端如何向服务器请求 Web page，其底层使用可靠的 TCP 运输协议，所以不用担心请求的过程数据在中途丢失或出错的问题。
...
如果一个客户端与服务器的每一个 请求/响应 对，分别由单独的 TCP 连接发送，这样的 Web 应用程序称为 **非持续连接（non-persistent connection）**。假设在一个由 1 个 index.html 和 10 张 jpg 图片组成的 Web page 中传输，则会建立 11 个 TCP 连接。
...
比如说，当我们第一次登录 JD.com 进行购物的时候，JD服务器的 response 报文会对我的浏览器设置一个 `set-cookie: 1678` 的首部行，这个 `set-cookie` 把 cookie 值存入了我们的电脑。然后我们点击几样商品，转入购物车结算页面，此时对 HTTP 来说是一次全新的连接，但是结算页面却能准确显示我们刚才选的商品，这是因为我们在进入结算页面时，request首部中包含了刚刚 JD.com 给我们的 `cookie: 1678`， JD.com 接收到这个 cookie ，就知道你是刚刚 1678 那个人了，于是把刚才页面你勾选的商品显示出来，进行结算。
...
也就是说，URI 分为三种，`URL` or `URN` or `（URL and URN）`

## HTTP 中 URL 的参数

如果我们

1. 访问知乎：`https://www.zhihu.com/`
2. 搜索“Spring”，回车。

会发现，浏览器的地址变成了：`https://www.zhihu.com/search?type=content&q=Spring`
...
比较形象的描述：HTTP是轿车，提供了封装或者显示数据的具体形式，你直接开就是；Socket是发动机，提供了网络通信的能力，你需要自己组装成一辆车来开。
...
SSE 是一种允许服务端通过单向 HTTP 连接向客户端主动推送数据的技术。运行在现有的 HTTP 协议之上，不需要像 WebSocket 那样升级协议。

在传统的 HTTP 模型中，客户端必须发送请求，服务端才能响应。而通过 SSE，客户端只需建立一次初始的 HTTP 连接，服务端就可以在保持连接打开的状态下，持续不断地将数据流推送给客户端。

在技术实现上，SSE 依赖于 HTML5 标准中的 `EventSource` API。服务端需要将 HTTP 响应的 `Content-Type` 设置为 `text/event-stream`，然后以特定的文本格式（通常包含 `data:`、`id:`、`event:` 等字段）持续发送数据。

适合 SSE 的场景：
- 实时股票/加密货币价格推送
- 社交媒体信息流的新消息通知
- 人工智能（AI） 模型的流式输出（例如大语言模型生成文本的打字机效果）
- CI/CD 构建日志的实时监控
