...
索引是帮助提高查询效率的数据结构。从底层实现的角度，索引包括 BTree索引 和 Hash索引，InnoDB和MyISAM只支持BTree索引，Memory两种都支持。从功能的角度，索引分为普通索引、唯一索引、组合索引、空间索引、全文索引。从结构组织的角度，索引分为聚簇索引和非聚簇索引。
...
非必要不分。在分之前，先尝试SQL优化、引入Redis缓存、升级硬件、读写分离、历史数据归档。

**如果数据量大，并发高，优先考虑 TiDB、OceanBase 等分布式 NewSQL 数据库。**

如果真的要分：

垂直拆分：

- 垂直分库：按照业务模块把库拆分到不同的数据库实例上，与微服务改造同步进行。
- 垂直分表：对字段非常多的大表，将核心的、高频访问的字段放在一张主表中，将长文本或低频访问的扩展字段拆分到另一张扩展表中。

垂直拆分的缺点是会引起Join操作，事务变得更加复杂，可以通过在应用层进行Join来解决。

水平拆分：

把一个很多行的表拆分到不同的分表和分库中去。常见的做法有**客户端代理**（如ShardingSphere）和 **中间件代理**（如MyCAT）。

水平拆分的方式：

- **哈希取模（Hash）**：用某个核心字段（如 user_id）进行 Hash 然后取模（例如 user_id % 32）。好处是数据分布均匀，不会产生热点。缺点是后续扩容极其痛苦，通常需要进行复杂的数据成倍迁移。
- **范围划分（Range）**：按照时间（按月/年）或 ID 范围（如 1~100万，100万~200万）进行拆分。好处是扩容平滑。缺点是容易产生热点数据。
...
- 分布式事务问题（需引入基于消息队列的最终一致性方案）
- 跨库 JOIN （应用层JOIN）
- 分页与排序问题（可能会出现内存排序）
- 多维度查询难题（如果按user_id分片， 用shop_id查询困难，需引入 ES 等查询系统）
